...
const calculateCardStats = (char) => {
    ...
    // Define se a carta aguenta o tranco
    let survival = 20;
    if (char.status === 'Alive') {survival = getRandom(90, 100);}
    else if (char.status === 'unknown')  {survival = getRandom(50, 70);}
    else if (char.status === 'Dead')  {survival = getRandom(10, 30);}
    ...
    const totalPower = cap(survival) + cap(popularity) + cap(weirdness) + protagonistBonus;
    
    // Definição das Classes
    let rarity = 'COMMON';
    
    if (totalPower >= 240) rarity = 'LEGENDARY'; // ~5% das cartas (Rick, Morty)
    else if (totalPower >= 180) rarity = 'EPIC'; // ~15% (Birdperson, Summer)
    else if (totalPower >= 120) rarity = 'RARE'; // ~30% (Aliens fortes)
    else rarity = 'COMMON';                      // ~50% (Humanos mortos/figurantes)
