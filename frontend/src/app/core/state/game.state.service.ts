...
export class GameStateService {
  ...
  readonly isDataReady = computed(() => this._allCards().length >= 4);
  ...
  getRandomCards(amount: number): Card[] {
    ...
    // Embaralhamento robusto (Fisher-Yates) para aleatoriedade uniforme e confiável
    ...
    if (result.length < amount) {
      console.warn(`[GameStateService] Solicitado ${amount} cartas, mas apenas ${result.length} estão disponíveis.`);
    }
